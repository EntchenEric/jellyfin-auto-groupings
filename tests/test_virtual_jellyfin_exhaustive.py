...
def test_add_virtual_folder_409_conflict(jellyfin_url) -> None:
...
def test_add_virtual_folder_500_create(jellyfin_url) -> None:
    with pytest.raises(RuntimeError) as excinfo:
...
def test_add_virtual_folder_400_paths(jellyfin_url) -> None:
    with pytest.raises(RuntimeError) as excinfo:
...
def test_add_virtual_folder_502_refresh(jellyfin_url) -> None:
    with pytest.raises(RuntimeError) as excinfo:
        add_virtual_folder(
            jellyfin_url,
            "FAIL_REFRESH_KEY",
...
def test_add_virtual_folder_invalid_collection(jellyfin_url) -> None:
...
def test_add_virtual_folder_empty_paths(jellyfin_url) -> None:
    # Should not fail.
...
def test_delete_virtual_folder_404(jellyfin_url, caplog) -> None:
...
def test_delete_virtual_folder_500(jellyfin_url) -> None:
    with pytest.raises(RuntimeError):
...
def test_get_library_id_missing(jellyfin_url) -> None:
    lib_id = get_library_id(jellyfin_url, TEST_API_KEY, "DoesNotExist")
    assert lib_id is None


def test_get_library_id_missing_itemid_key(jellyfin_url) -> None:
    lib_id = get_library_id(jellyfin_url, "LIB_GET_MISSING_ID", "Movies")
    assert lib_id is None


def test_get_library_id_500(jellyfin_url) -> None:
    with pytest.raises(RuntimeError) as excinfo:
...
def test_set_virtual_folder_image_missing_id(jellyfin_url, tmp_path, caplog) -> None:
...
def test_set_virtual_folder_image_oserror(jellyfin_url, caplog) -> None:
    set_virtual_folder_image(
        jellyfin_url,
        TEST_API_KEY,
        "Movies",
...
def test_set_virtual_folder_image_unknown_mime(jellyfin_url, tmp_path, caplog) -> None:
    caplog.set_level(logging.INFO)
...
def test_set_virtual_folder_image_400(jellyfin_url, tmp_path, caplog) -> None:
    caplog.set_level(logging.INFO)
    img_path = tmp_path / "test.jpg"
    img_path.write_bytes(b"data")
...
def test_get_users_500(jellyfin_url) -> None:
    with pytest.raises(RuntimeError) as excinfo:
        get_users(jellyfin_url, "USER_GET_500")
    assert excinfo.value.__cause__.response.status_code == 500


def test_get_user_recent_items_bad_user(jellyfin_url) -> None:
    items = get_user_recent_items(jellyfin_url, TEST_API_KEY, "BAD_USER")
    assert items == []


def test_get_user_recent_items_missing_data(jellyfin_url) -> None:
